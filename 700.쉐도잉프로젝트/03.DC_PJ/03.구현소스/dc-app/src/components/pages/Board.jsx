...
function Board() {
  ...
  // [2] 전체 레코드 개수(배열 데이터 개수) 
  // - 매번 계산하지 않도록 참조변수로 생성한다!
  const totalCount = useRef(baseData.length);
  console.log('전체개수:',totalCount)

  // 일반 변수 셋팅구역 : 매번 같은 값을 유지해야하는 변수들
  // [1] 페이지당 개수 : 페이지당 레코드 수
  const unitSize = 5;
  ...
  //  페이지 시작번호 : 단위 수 * 페이지 번호 - 1
  let initNum = unitSize*(pageNum-1);
  // 한계수 번호 : 단위 수 * 페이지 번호
  let limitNum = unitSize*pageNum;
  // 샘플계산 (단위수 5, 1~3)
  // 시작 수 (5 * (1-1)) = 0 / 한계수(5*1) = 5
  // 시작 수 (5 * (2-1)) = 5 / 한계수(5*2) = 10
  // 시작 수 (5 * (3-1)) = 10 / 한계수(5*3) = 15
