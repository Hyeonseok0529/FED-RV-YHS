...
Vue.component('de-fashion-list',{
    ...
    props:['list-idx','list-tit','list-price','fn-add-comma',
        //케밥 케이스를 케믈케이스로 변경하여 변수/함수로 사용가능!
        //listIdx, listTit, listPrice, fnAddComma 로 사용가능하다는 말
        // 실제로 사용할 때는 this키워드를 사용하여 객체내부용으로 씀.
,    ],
...
new Vue({
    ...
    // 부모 뷰인스턴스 데이터구역 //
    data:{
        // 부모 컴포넌트에서 사용할 import한 데이터 할당!
        exData: deData,
        // 하위 컴포넌트를 데이터만큼 순회할 때 여기서 불러온다!
