...
const store = new Vuex.Store({
    ...
    actions: {
      ...
      getData() {
        ...
        // 데이터파일인 제이슨 파일을 불러온다!
        // -> 당연히 엑시오스 라이브러리를 불러오면 사용 가능
        // -> 엑시오스 이름은 axios 로 정해져 있다 

        // ((구문))
        // axios.get(파일경로).then(result=>{코드})
        // -> 앞의 get 메서드로 파일을 로딩하고
        ...
        // JS의 try/cat구문처리와 유사함!

        // 제이슨 파일이 그 크기가 커서 시간이 걸려도
        // 엑시오스 구문을 사용하면 데이터 로딩 후 
        // 다음 메서드를 지연하고 있다가 로딩 후 호출한다!
        // 이런 호출방식을 비동기 호출 방식이라고 한다!
        // JS에서 프라미스 방식으로 불리기도 함!
