...
////////// 로딩구역 //////////
// 로딩관련 이벤트는 2가지!
// 1) load : 모든 컨텐츠 내용이 로딩 된 후 실행
// -> 이미지나 모든 시간이 걸리는 컨텐츠도
// 모두 로딩 될떄까지 기다림!
// 2) DOMContentLoaded : 태그만 다그리면 실행 (DOM Content 는 : html 태그를 뜻한다.)
...
function loadFn() {
  ...
  function controlTV() {
    ...
    // 아이디명도 특별하므로 id로 가져옴
    // 참고)class라고 못하는 이유는 class가 따로 있기때문
    ...
    // [ CSS 스타일값 읽기방법 2가지]
    // 1) style.속성명 방법 (인라인 CSS만 읽어옴)
    // -> 왜 인라인만 읽어오는가?  style.속성명=값 설정을
    // 생각해보면 이것이 인라인 CSS를 설정하는 방법이기 때문이다.
    // 2) getComputedStyle(대상요소).속성명 방법
    // -> 2번 방법은 실제로 화면에 셋팅된 CSS 값을 읽어옴
