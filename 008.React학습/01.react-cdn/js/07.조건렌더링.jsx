...
// 음식리스트 배열변수 //
const foods = ["스파게티", "짜파게티", "냉면", "짜장면", "마라탕","마라샹궈","탕수육"];
    ...
    return(
      <li>개발자는
        ...
        }
        {
          ...
          // false 처리됨! 할당되면 true 처리됨!
            movieInfo && 
            "영화 개봉년도 , 영화제목:"+movieInfo.year + movieInfo.mtit
          // movieInfo는 객체 데이터
          // 하위속성으로 year는 년도 데이터
          // mtit는 영화제목 데이터
        }
...
function WishList({wList, gubun}){ // 변수는 대문자로 시작하면 안됨!
  ...
  return (
    <React.Fragment>
      ...
      wList.length > 0 &&
        ...
        gubun == "음식" &&
        <h2>
        개발자가 좋아하는 음식은 모두 {wList.length}가지 입니다.
      </h2>
          }
          {// 구분값이 "영화"인 경우
        gubun == "영화" &&
      ...
      {
        wList.length == 0 && (
          <h2> 개발자 {gubun} 리스트가 업데이트 되지 않았습니다.</h2>
        )
      }
