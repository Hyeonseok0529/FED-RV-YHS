...
// 리액트에서는 CDN에서 from 뒤에 파일명에 "js || jsx" 생략가능
/************************************************* 
    [ 리액트 컴포넌트 ]
    ...
    [ 컴포넌트의 호출 ]
     ...
     2. 전달값은 태그의 속성셋팅과 방식으로 보내준다
     예) function HaHaHa(){return 코드}
     {return 코드}
...
function MakeImage(변수헐) {
  console.log("전달객체:", 변수헐);
  // 컴포넌트에 전달되는 값은 객체로 전달된다!
  // {ialt, isrc} - 구조분해할당
  // 객체가 들어올 때 해당 속성명으로 받으면 된다!
  return (
    <figure>
      <img src={"./images/" + 변수헐.isrc} alt={변수헐.ialt} />
      <figcaption>{변수헐.ialt}</figcaption>
    </figure>
  );
} /////// MakeImage 컴포넌트 ////////
...
function FavoriteThings(조아) {
  return (
    <h2>
      좋아하는 색 : <span style={{ color: "skyblue" }}>{조아.color}</span>
      <br />
      좋아하는 음식 : {조아.food} <br />
      취미 : {조아.hobby} <br />
    </h2>
