...
myFn.addEvt(sel3, "change", function () {
  ...
  // const newArray = list1; // 이거는 얕은 복사 (주소지 복사)

  // -> 객체 데이터를 변경하는 경우엔
  // 위의 깊은 복사가 아닌 JASON.parse()방식을 써야한다!
  //    newArray[0].idx = 999;
...
console.log('like검색기초(indexOf)대상문자의 문자순번"당":',list2
[0].tit.indexOf('당'));
console.log('like검색기초(indexOf)대상문자의 문자순번"가":',list2
[0].tit.indexOf('가'));
console.log('like검색기초(indexOf)대상문자의 문자순번"헐":',list2
[0].tit.indexOf('헐'));
...
console.log('검색테스트3(filter)"당"이 있는 제목:', searchText3);
...
console.log('검색테스트3(filter)"다"이 있는 제목:', searchText4);

// 검색 테스트 5
let searchText5 = list2.filter(v=>{
  if(v.tit.indexOf('읭')!==-1) return true;
})

console.log('검색테스트3(filter)"읭"이 있는 제목:', searchText5);
// 데이터가 없으면 빈배열을 리턴함
// 따라서 없다는 것은 배열길이가 0이라는 말
// 배열.length==0 이 값이 true면 검색결과가 없는 것!

// indexOf 말고 배열값 중 어떤 값을 포함하는지 여부를
// 알아내는 함수는 include(값) -> 있으면 true, 없으면 false

// indexOf()와 include()는 모두 배열에서도 사용하고
// 문자열(string) 에서도 사용하는 메서드이다!
// 여기서는 배열값 중 특정 문자열값에서 찾는 역할을 한다!

console.log("찾을대상:",list2[0].tit);
console.log("include('당'):",list2[0].tit.includes('당'));
console.log("include('멍'):",list2[0].tit.includes('멍'));
