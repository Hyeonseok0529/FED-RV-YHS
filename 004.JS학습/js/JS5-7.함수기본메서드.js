...
// 출력 코딩구역 //////////////////
  ...
  // HTML요소도 객체의 일종이다!
  // 선택요소.title, 선택요소.id 이렇게 객체값을 찍는다! -> 이 자체가 객체라는 말.


  // 4. Math.max.apply() / Math.max.apply() 사용하기
  // apply(객체,배열) -> 앞의 객체값을 불필요로 하기 때문에 null 보냄
  // 뒤에 배열만 필요하고 이것을 보내서 최대값/최소값을 리턴함
  // 단순배열
  const myNum = [2400, 3600, 4200, 5300, 6700];
   exBox[6].innerText =
   myNum+"의 최대값:"+Math.max.apply(null,myNum)
  ...
  */
  ...
  // map돌린 배열
  console.log("map돌린배열:",myObj.map((v) => v.pay));
  ...
  // ->bind(객체)는 객체를 묶어서 보내는 역할, 그 뒤에 소괄호()를 넣어서 실행해준다!
