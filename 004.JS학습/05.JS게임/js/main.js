...
function goR1(){
    ...
    // 인터발변수에 할당하여 멈출 수 있게 함
    // 이때 변수할당전에는 변수가 undefined 이므로
    // if문으로 한 번 할당후엔 실행 못하게 막아준다!
    if(!autoI){ // 할당전 false일때 !(Not연산자) true로 변경
        // 인터발호출
        autoI = setInterval(() => {
            // 1) 토끼 위치값 변수 1씩 증가
            r1pos++;
...
function whoWinner(){
    ...
    if (r1pos >= FINAL_NUM || t1pos >= FINAL_NUM){
        ...
        // (9) 버튼박스 위로 올리기
        myFn.qs('#btns').style.zIndex = 200; // <- 숫자로만 이루어진 값이면 ''로 안싸도 가능
