...
// 새로고침시 스크롤위치값 캐싱때문에 강제로 위로가기 설정
setTimeout(() =>window.scrollTo(0,0),0)
// setTimeout 으로 감싸면 스텍의 모든 코드 실행후 나중에 실행됨
// 이때 스크롤 캐싱도 적용 후에 실행하게 됨. 즉 덮어씀! 성공!
...
// (1-2) body에 overflow-hidden 설정
myFn.qs("body").style.overflow = "hidden";

// 2. 전역변수 설정하기 ////////////////////////
// (2-1) 페이지변수
let pgNum = 0;
// (2-2) 휠 상태변수 (광휠금지상태: true - 막기 , false - 허용)
let stsWheel = false;
// (2-3) 각 페이지요소 수집 (.page인 요소들)
const pageEl = myFn.qsa(".page");
// (2-4) 전체 페이지 수 상수설정 / 상수를 담는 변수는 전체 대문자를 써야함!
...
function wheelFn(e) {
  ...
  // x 축은 0, y축은 해당 순번의 .page박스 위치값 넣음
...
function movePage(evt, el, idx, list) {
  // console.log('클릭!',this,this.getAttribute('href'));
  // e - 이벤트 전달변수
  // (함수와 직접연결된 경우 자동전달됨!)
  // (그러나... 호출되는 일반함수일경우 전달해야함!)
